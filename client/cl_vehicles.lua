-- To rewrite some time later!

print(('%sLoaded vehicle module, to be rewritten at a later date!'):format(FRAMEWORK.DebugPrint))
...
AddEventHandler('echorp:getvehicledata', function(vehicle, cb)
    ...
    if DoesEntityExist(vehicle) then
        ...
        VehicleData['plate'] = FRAMEWORK.UTILS:MathTrim(GetVehicleNumberPlateText(vehicle))
        VehicleData['plateIndex'] = GetVehicleNumberPlateTextIndex(vehicle)

        VehicleData['bodyHealth'] = FRAMEWORK.UTILS:MathRound(GetVehicleBodyHealth(vehicle), 1)
        VehicleData['engineHealth'] = FRAMEWORK.UTILS:MathRound(GetVehicleEngineHealth(vehicle), 1)
        VehicleData['petrolHealth'] = FRAMEWORK.UTILS:MathRound(GetVehiclePetrolTankHealth(vehicle), 1)

        VehicleData['dirtLevel'] = FRAMEWORK.UTILS:MathRound(GetVehicleDirtLevel(vehicle), 1)
